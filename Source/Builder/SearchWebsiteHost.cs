...
{
    internal sealed class SearchWebsiteHost : IDisposable
    {
        #region Static methods

        internal static async Task<SearchWebsiteHost> EnsureAvailableAsync(
            string projectPath,
        ...
        {
            SearchLaunchProfile launchProfile = LoadLaunchProfile(
                launchSettingsPath,
            ...
            if (await IsAvailableAsync(launchProfile.BaseUri).ConfigureAwait(false))
            {
                ...
                return new SearchWebsiteHost(launchProfile.BaseUri, null);
            }
            ...
            try
            {
                ...
                return new SearchWebsiteHost(launchProfile.BaseUri, process);
            }
            catch
            {
                StopProcess(process);
        ...
        private static SearchLaunchProfile LoadLaunchProfile(
            string launchSettingsPath,
            string preferredLaunchProfile,
            string? fallbackLaunchProfile
        )
        {
            ...
            return new SearchLaunchProfile(profileName, baseUri);
        }
        ...
        private SearchWebsiteHost(Uri baseUri, Process? process)
        {
            BaseUri = baseUri;
            _process = process;
        }
    ...
    internal sealed record SearchLaunchProfile(string Name, Uri BaseUri);
}