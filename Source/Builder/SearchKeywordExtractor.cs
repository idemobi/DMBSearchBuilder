...
namespace DMBSearchBuilder
{
    ...
    /// </summary>
    public static class SearchKeywordExtractor
    {
        ...
        {
            IReadOnlyList<string> tokens = SearchTextNormalizer.ExtractSearchTokens(content);
        ...
        private static Dictionary<string, int> CountTokenOccurrences(string content)
        {
            ...
            string normalized = SearchTextNormalizer.NormalizeSearchText(decodedContent);
        ...
        {
            string decodedContent = WebUtility.HtmlDecode(content) ?? string.Empty;
            return string.Join(' ', SearchTextNormalizer.ExtractSearchTokens(decodedContent).Take(250));
        }
        ...
        /// <returns>A normalized <see cref="SearchPageRecord" /> ready for storage.</returns>
        public static SearchPageRecord ExtractPage(string url, string html)
        {
