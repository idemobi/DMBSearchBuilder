...
namespace DMBSearchBuilder
{
    ...
    /// </summary>
    public sealed class SearchBuilderAgent
    {
        ...
        public async Task<int> BuildAsync(SearchBuildOptions options, CancellationToken cancellationToken = default)
        {
            ...
            while (queue.Count > 0 && savedCount < options.MaxPages)
            {
                ...
                SearchPageRecord record = SearchKeywordExtractor.ExtractPage(BuildStoredUrl(pageUri), html);
                records.Add(record);
                savedCount++;
            ...
            if (records.Count > 0)
            {
                SearchDatabaseManager.ClearPages(options.DatabasePath);

                foreach (SearchPageRecord record in records)
                {
                    SearchDatabaseManager.SavePage(options.DatabasePath, record);
                }
            }
            else
            {
        ...
        /// <returns>The number of pages written to the search database.</returns>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="options" /> is <see langword="null" />.</exception>
        public async Task<int> BuildFromLaunchProfileAsync(SearchLaunchProfileBuildOptions options, CancellationToken cancellationToken = default)
        {
            options = options ?? throw new ArgumentNullException(nameof(options));

            using SearchWebsiteHost websiteHost = await SearchWebsiteHost.EnsureAvailableAsync(
                options.ProjectPath,
            ...
            SearchBuildOptions buildOptions = new()
            {
